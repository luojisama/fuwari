		...
		if (!isManual) {
			...
			try {
				...
				if (tencentRes && tencentRes.status === 0) {
					...
					detailedAddress =
						`${adInfo.province} ${adInfo.city} ${adInfo.district}`.trim();
			...
			try {
				...
				if (addressRes.ok) {
					...
					address =
						...
						address;
					detailedAddress =
						`${addressData.address.state || ""} ${addressData.address.city || ""} ${addressData.address.district || ""}`.trim();
				} else {
				...
				try {
					...
					address =
						bdcData.city || bdcData.principalSubdivision || bdcData.countryName;
					detailedAddress =
						`${bdcData.principalSubdivision || ""} ${bdcData.city || ""} ${bdcData.locality || ""}`.trim();
				} catch (e2) {
